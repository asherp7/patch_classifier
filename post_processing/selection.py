...
def get_filenames(ct_dir_path, tumor_dir_path, cnn_pred_dir_path, chan_vase_dir_path):
    filenames = []
    for ct_filename in os.listdir(ct_dir_path):
...
def create_random_forest_features(filenames):
    feature_list = []
    for ct_filename, tumor_filename, cnn_pred_filename, chan_vase_filename in filenames:
        ct_data = nib.load(ct_filename)
        cnn_pred_data = nib.load(cnn_pred_filename)
        chan_vase_data = nib.load(chan_vase_filename)
        feature_list, _, _ = get_scan_connected_componenets_features_and_labeled_array(ct_data,
                                                                                       cnn_pred_data,
                                                                                       chan_vase_data)
        feature_list.extend(feature_list)
    return np.array(feature_list)


def create_random_forest_labels(filenames):
    gt_list = []
    for _, tumor_filename, _, chan_vase_filename in filenames:
        chan_vase_data = nib.load(chan_vase_filename)
        tumor_data = nib.load(tumor_filename)
...
if __name__ == '__main__':
    ct_dir_path = '/cs/labs/josko/asherp7/combined_data/ct_scans'
    tumor_dir_path = '/cs/labs/josko/asherp7/combined_data/liver_seg'
    cnn_pred_dir_path = '/cs/labs/josko/asherp7/follow_up/outputs/results_27_2/predictions'
    chan_vase_dir_path = '/cs/labs/josko/asherp7/follow_up/outputs/results_27_2/chan_vese_remove_small_cc'
    ...
    filenames = get_filenames(ct_dir_path, tumor_dir_path, cnn_pred_dir_path, chan_vase_dir_path)
    # gt = create_random_forest_labels(filenames)
    # features = create_random_forest_features(filenames)
    # num_estimatores = 30
    # classifier = train_random_forest(num_estimatores, features, gt)